# Voice_Diary
Данное приложение создано в ходе выполнения тестового задания на стажировку в VK.
...
Изначально я планировал реализовать больший функционал в приложении. Приведу перечень того, что я внедрить не успел:

 * Постановка воспроизводимой записи на паузу и продолжение её воспроизведения после снятия с паузы. В текущей версии воспроизведение аудиозаписей всегда начинается с самого начала;
 * Отображение текущей позиции (в контесксте воспроизведения) воспроизводимой аудиозаписи;
 * Удаление аудио и переименование аудио - для этого уже были реализованы соответствующие методы в FileManager, однако на их внедрение в UI времени не хватило. SwipeToDelete так и остался нереализованным, хотя ItemTouchHelper.SimpleCallback для этого уже был реализован;
 * Загрузка аудио в VK Документы - для этого уже была реализована авторизация в VK. 
   Планировалось внедрить несколько способов для загрузки аудиофайлов:
    * Автоматическая загрузка, после записи аудио. Этот способ мог бы отключаться в настройках. Для загрузки единичного аудиофайла можно было бы использовать корутины.
    * По долгому нажатию на элемент списка происходил бы переход в selection mode, где можно было бы выбрать аудиофайлы для загрузки. После этого можно было бы нажать на кнопку ActionBar и начать загрузку аудиофайлов. При этом для загрузки более 1 файла использовался бы foreground service с соответствующим уведомлением.
    * Соответствующая кнопка в настройках таке переносила бы пользователя на RecordsFragment и включала бы selection mode.

### Пути развития приложения
Помимо этого, при разработке я выделил еще ряд возможностей, которые хорошо бы вписались в функционал приложения. Я не планировал их реализовывать ввиду ограничения по времени на разработку, однако считаю, что о них стоит упомянуть:

 1. **Пагинация при загрузке списка файлов.** Я заметил, что уже при относительном небольшом количестве файлов время загрузки списка достаточно велико. Внедрение пагинации помогло бы устранить эту проблему.
 2. **Воспроизведение аудио в фоне.** Аудиофайлы могут иметь большую продолжительность, поэтому хорошим решением было бы их воспроизведение в фоне, с использованием foreground service.
 3. **Использовать wake lock при записи аудио.**
